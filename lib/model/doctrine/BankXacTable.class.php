 ...
 */
class BankXacTable extends Doctrine_Table
{
    ...
    # XAC
//    const XAC_URL = 'http://192.168.100.74:8080/InfoService/services/StatementService?wsdl';
    const XAC_URL = 'http://192.168.100.76:80/InfoService/services/StatementService?wsdl';
    #type
    CONST TYPE_DEALER = 1;
     ...
     *
     * @return object BankXacTable
     */
    public static function getInstance()
    {
        return Doctrine_Core::getTable('BankXac');
    }
     ...
     * @return BankXac
     */
    public static function findOneByOrderId($orderId, $bankId = VendorTable::BANK_XAC)
    {
        $q = Doctrine_Query::create()
                ->from('BankXac')
                ->where('order_id = ?', $orderId)
                ->addWhere('vendor_id = ?', $bankId);
        if ($bankId == VendorTable::BANK_XAC) {
...
//         DATE_FORMAT(o.outcome_date, '%Y-%m-%d')
    ...
    /**
     * Сүүлийн гүйлгээний ID олох
     * @param $acountNumber
     * @return BankXac
     * @throws Doctrine_Manager_Exception
     */
    public static function getLastTransictionId($acountNumber)
    {
        $query = "
SELECT order_sequence_no FROM bank_xac b
 WHERE (b.bank_account = '" . $acountNumber . "') ORDER BY b.order_sequence_no desc
LIMIT 1";
        $pdo = Doctrine_Manager::connection()->getDbh();
        $row = $pdo->query($query)->fetch(PDO::FETCH_ASSOC);

        if (count($row) && $row['order_sequence_no']) {
            return $row['order_sequence_no'];
        }
        return 1;
    }
    ...
    public static function getList($account, $excel = false)
    {
        ...
        $q->addWhere('vendor_id = ?', VendorTable::BANK_XAC)
        ...
        $pager = new sfDoctrinePager('BankXac', 100);
        $pager->setQuery($q);
        $pager->setPage($page);
        $pager->init();
    ...
    public static function getListCustom($account, $excel = false)
    {
        ...
        $where[] = "vendor_id=" . VendorTable::BANK_XAC;
    ...
    public static function insert($trans)
    {
        try {
            ...
            $bankOrder->order_sequence_no = $trans['SequenceNo'];
            ...
            $logger = new sfFileLogger(new sfEventDispatcher(), array('file' => sfConfig::get('sf_log_dir') . '/bankPaymentLog/xac/bankPaymentLog_'.date('Y-m-d').'.log'));
            $logger->log('$insert XAC Error: '.$bankOrder->order_id .', Error message: '.$exc->getMessage(), sfFileLogger::ERR);
        }
    ...
    public static function init()
    {
        ...
        $errorCode = -1;
        $desc = 'None';
        $request = $url = self::XAC_URL;
        $transactions = array();
        $response = '-';

        $acountNumbers = BankXacAccountTable::getList(TRUE);
        foreach ($acountNumbers as $acountNumber) {

            try {
                $logId = LogTools::setLogXacInit($request);

                $client = new SoapClient($url, array('cache_wsdl' => WSDL_CACHE_NONE));

                $sequence = self::getLastTransictionId($acountNumber);

                $desc = $sequence;

                $yaml = sfYaml::load(sfConfig::get('sf_config_dir') . '/app_banks.yml');
                $xacPassToDec = $yaml['all']['bankXacPass'];
                $xacPass = AppTools::passDecrypt($xacPassToDec);

                $xacUserId = $yaml['all']['bankXacUserId'];
                $xacUser = $yaml['all']['bankXacUser'];

                $params = array(
                    "AccountNo" => $acountNumber,
                    "MemberCode" => $xacUserId,
                    "Organization" => $xacUser,
                    "Password" => $xacPass,
                    "Sequence" => $sequence
                );
                $requestParams = print_r($params, true);

                $obj = $client->Request($params);

                $response = print_r($obj, true);
                $arr = self::object_to_array($obj);
                $errorCode = 0;
                $getSuccess = 1;
                $transactions = $arr['Statements'];
            } catch (SoapFault $e) {
                $errorCode = 100;
                $desc = $e->faultstring;
                $responseSet = print_r($desc, true);
            }
            //$responseSet = print_r($transactions, true);
            LogTools::setLogXacInitUpdate($logId, array(
            ...
            if (count($transactions)) {
                ...
                $logger = new sfFileLogger(new sfEventDispatcher(), array('file' => sfConfig::get('sf_log_dir') . '/my-xac-order.log'));
                $logger->log('--START--=', sfFileLogger::INFO);
                foreach ($transactions as $trans) {
                    $param = array();
                    $param['TxnType'] = $trans['DrcrInd'];
                    $param['Account'] = $trans['AccountNo'];
                    $param['JournalNo'] = $trans['TrnRefNo'];
                    $param['TxnDesc'] = AppTools::cp1251_utf8($trans['Narration']);
                    $param['Amount'] = (double) $trans['Amount'];
                    $param['Branch'] = $trans['BranchNo'];
                    $param['Channel'] = $trans['Channel'];
                    $param['SequenceNo'] = $trans['SequenceNo'];
//                    $param['TxnDate'] = date('Y-m-d H:i:s', strtotime($trans['TrnTime']));
                    $param['TxnDate'] = date('Y-m-d', strtotime($trans['TrnDate']));
                    try {
                        $bankOrder = BankXacTable::insert($param);
                        $logger->log('SUCCESS' . $param['JournalNo'], sfFileLogger::INFO);
                    } catch (\Exception $exc) {
                        $logger = new sfFileLogger(new sfEventDispatcher(), array('file' => sfConfig::get('sf_log_dir') . '/my-xac-order.log'));
                        $logger->log('--ERROR--=' . trim($param['JournalNo']) . $exc->getMessage(), sfFileLogger::ERR);
                    }
                    $setSuccess = 1;
                }
            } else {
    ...
    {
        $customer = FALSE;
        if ($bankOrder instanceof BankXac) {
            ...
            $bankOrderRows = self::updateForCharge(array(BankXacAccountTable::ACCOUNT_DEALER), $limit);
        }
...
//                    print_r($dealerAgent);die();
                    ...
                    if ($dealerAgent || $dealer || $adCode) {
                        $logId = LogTools::setLogXacCharge($bankOrder->id);
                        $orderMobile = null;
                        unset($chargeResult);
                        $time = -time();
                        # Цэнэглэлт хийх
                        if ($SD) {
                            $chargeResult = BaseSms::chargeSdDealer($bankOrder->charge_mobile, $bankOrder->order_amount, 'XAC');
                        } elseif ($ADshop) {
                            $chargeResult = BaseSms::chargeAdDealer($adCode, $bankOrder->order_amount, 'XAC');
                        } elseif ($dealerAgent) {
                            $chargeResult = DealerGateway::charge($dealerAgent['dealerId'], $bankOrder->order_amount, 'XAC', $logger);
                        ...
                        LogTools::updateLogXacCharge($logId, $chargeResult['log_request'], $chargeResult['log_response'], $time);
    ...
    public static function rechargeSMSApi($bankOrder, $type)
    {
        ...
        $time = -time();
        if ($type == "SD") {
            $chargeResult = BaseSms::chargeSdDealer(str_replace("SD", "", $bankOrder->charge_mobile), $bankOrder->order_amount, 'XAC');
        } elseif ($type == "AD") {
            $chargeResult = BaseSms::chargeAdDealer($bankOrder->charge_mobile, $bankOrder->order_amount, 'XAC');
        }
        $time += time();
        LogTools::updateLogXacCharge($logId, $chargeResult['log_request'], $chargeResult['log_response'], $time);
...
//                Шалтгаан: <b>' . self::getStatusName($bankOrder->status, BankXacTable::TYPE_DEALER) . '</b><br><br>
...
//        LogTools::setLogEmail($subject, $body, $from, $to);
    ...
    {
        $whichLog = '';
        if($account == array(BankXacAccountTable::ACCOUNT_USSD_TOPUP)){
            $whichLog = '/topupSapc/xac/topupSapc_'.date('Y-m-d').'.log';
        }else if($account == array(BankXacAccountTable::ACCOUNT_MOBINET_PAYMENT, BankXacAccountTable::ACCOUNT_CALLPAYMENT)){
            $whichLog = '/bankPaymentLog/xac/bankPaymentLog_'.date('Y-m-d').'.log';
        }else if($account == array(BankXacAccountTable::ACCOUNT_PRODUCT_MOBINET)){
            $whichLog = '/bankPaymentHBB/xac/bankPaymentLog_'.date('Y-m-d').'.log';
        }
    ...
    public static function callPayment($bankOrder = null, $limit = 50)
    {
        if ($bankOrder instanceof BankXac) {
            ...
            $bankOrderRows = self::updateForCharge(array(BankXacAccountTable::ACCOUNT_CALLPAYMENT), $limit);
        }
        ...
        foreach ($bankOrderRows as $bankOrder) {
            try {
                ...
                if ($bankOrder->status == self::STAT_PROCESS) {
                    ...
                    $logId = LogTools::setLogXacChargeCallPayment($bankOrder->id);
                    ...
                    LogTools::updateLogXacChargeCallPayment($logId, $chargeResult['log_request'], $chargeResult['log_response'], $time);
    ...
    public static function productPayment($bankOrder = null, $limit = 50)
    {
        if ($bankOrder instanceof BankXac) {
            ...
            $bankOrderRows = self::updateForCharge(array(BankXacAccountTable::ACCOUNT_PRODUCT_PAYMENT, BankXacAccountTable::ACCOUNT_PRODUCT_MOBINET, BankXacAccountTable::ACCOUNT_PRODUCT_ULUSNET, BankXacAccountTable::ACCOUNT_PRODUCT_BAG), $limit);
        }
    ...
    public static function setAssignment($limit)
    {
        ...
        $logger = new sfFileLogger(new sfEventDispatcher(), array('file' => sfConfig::get('sf_log_dir') . '/transaction/xac/transfer_'.date('Y-m-d').'.log'));
        $ids = array();
        foreach ($transactions as $transaction) {
            if ($transaction->status == self::STAT_SUCCESS && BankXacAccountTable::ACCOUNT_DEALER == $transaction->bank_account) {
            ...
            try {
                ...
                $logger->log($transaction->order_id . 'XACBANK' . $exc->getMessage(), sfFileLogger::ERR);
            }
        }
        if (count($ids)) {
            try {
              ...
              ";
                ...
                $logger->log('XACBANK-UPDATE' . $exc->getMessage(), sfFileLogger::ERR);
            }
        }
    }
    ...
    {
        $bankPaymentLogger = new sfFileLogger(new sfEventDispatcher(), array('file' => sfConfig::get('sf_log_dir') . '/bankPaymentLog/xac/bankPaymentLog_'.date('Y-m-d').'.log'));
        $bankPaymentLogger->log('============Started bankPayment XAC================', sfFileLogger::INFO);
        if ($bankOrder instanceof BankXac) {
            if ($bankOrder->canReCharge()) {
                $bankOrder->status = self::STAT_PROCESS;
                $bankOrder->save();
            ...
            $bankOrderRows = self::updateForCharge(array(BankXacAccountTable::ACCOUNT_MOBINET_PAYMENT, BankXacAccountTable::ACCOUNT_CALLPAYMENT), $limit);
        }
        ...
        foreach ($bankOrderRows as $bankOrder) {
            try {
                ...
                $type = 0;
                if ($bankOrder->getBankAccount() == BankXacAccountTable::ACCOUNT_CALLPAYMENT) {
                    $type = BankpaymentTable::TYPE_CALL_PAYMENT;
                    $limit = sfConfig::get('app_bankpayment_gsm_limit');
                } elseif (in_array($bankOrder->getBankAccount(), array(BankXacAccountTable::ACCOUNT_MOBINET_PAYMENT, BankXacAccountTable::ACCOUNT_PRODUCT_MOBINET))) {
                ...
                if (!$insertBankpayment) {
                    $bankPaymentLogger->log($bankOrder->order_id . ' burtgeegui bol burtgeh', sfFileLogger::INFO);
                    $data = array();
                    $data['vendor_id'] = VendorTable::BANK_XAC;
        ...
        $bankPaymentLogger->log('=============== Ended bankPayment XAC===============', sfFileLogger::INFO);
        return TRUE;
    }
    ...
    {
        $hbbLogger = new sfFileLogger(new sfEventDispatcher(), array('file' => sfConfig::get('sf_log_dir') . '/bankPaymentHBB/xac/bankPaymentHBB_'.date('Y-m-d').'.log'));
        $hbbLogger->log('============Started bankPaymentHBB XAC================', sfFileLogger::INFO);
        ...
        $hbbLogger->log('Total count: ' . count($bankOrderRows), sfFileLogger::INFO);
        echo 'bankPaymentHBB----'.count($bankOrderRows).'</br>';
        $count = 0;
        if(count($bankOrderRows)>0){
        foreach ($bankOrderRows as $bankOrder) {
            try {
                ...
                $hbbLogger->log($bankOrder->order_id . ' status:' . $bankOrder->status, sfFileLogger::INFO);
                if (in_array($bankOrder->status, array(self::STAT_FAILED_CHARGE, self::STAT_FAILED))) {
                    $bankOrder->status = self::STAT_PROCESS;
                    $bankOrder->save();
                }
                $hbbLogger->log($bankOrder->order_id . ' status1:' . $bankOrder->status, sfFileLogger::INFO);

                # MOBINET WIGI shalgah
                $wifiRegex = "/WIFI-(([9][954][0-9]{6})|(85[0-9]{6}))-([0-9_a-zA-Z]{1,20})-*/";
                if (preg_match($wifiRegex, $bankOrder->order_p, $matches)) {
                    $hbbLogger->log('--WIFI--=', sfFileLogger::INFO);
                    ...
                    $result = WifiOnlineGateway::chargeCard($phoneNumber, $card, "bankgw_xac");
                    //}
                    ...
                    $hbbLogger->log($bankOrder->order_id . ' $keyword: ' . $keyword, sfFileLogger::INFO);
                    $type = BankpaymentTable::TYPE_MOBINET_PREPAID;
                    $speed = 0;
                    if ($keyword) {
                        $accountInfo = MobinetGateway::contractInfo($keyword);
                        $hbbLogger->log($bankOrder->order_id . ' $accountInfo:' . print_r($accountInfo, true), sfFileLogger::INFO);
                ...
                $hbbLogger->log($bankOrder->order_id . ' status2:' . $bankOrder->status, sfFileLogger::INFO);
                if ($status == BankpaymentTable::STAT_BANKPAYMENT_TRANS_VALUE) {

                }
                # Hereglegch oldohgui bol HBB shalgah
                if ($status !== BankpaymentTable::STAT_NEW) {
                    $hbbLogger->log($bankOrder->order_id . ' status3:' . $bankOrder->status, sfFileLogger::INFO);
                    if ($redirectCall) {
                        return false;
                    } else {
                ...
                $hbbLogger->log($bankOrder->order_id . ' status4:' . $bankOrder->status, sfFileLogger::INFO);

                if (!$insertBankpayment) {
                    $hbbLogger->log($bankOrder->order_id . ' status5:' . $bankOrder->status, sfFileLogger::INFO);
                    try {
                        $data = array();
                        $data['vendor_id'] = VendorTable::BANK_XAC;
                        $data['bank_order_id'] = $bankOrder->id;
                        ...
                        $hbbLogger->log('$insertHBBBankpayment XAC Error: '.$bankOrder->order_id .' Error message: '.$exc, sfFileLogger::ERR);
                    }
                ...
                $bankOrder->status = self::STAT_FAILED;
                $bankOrder->save();
                $hbbLogger->log($bankOrder->order_id . ' Error: ' . $ex, sfFileLogger::INFO);
            }
        }
        }
        $hbbLogger->log('============Ended bankPaymentHBB XAC================', sfFileLogger::INFO);
        return TRUE;
    }
    ...
    {
        $topupLogger = new sfFileLogger(new sfEventDispatcher(), array('file' => sfConfig::get('sf_log_dir') . '/topupSapc/xac/topupSapc_'.date('Y-m-d').'.log'));
        $topupLogger->log('============Started bankPaymentTopupSapc XAC================', sfFileLogger::INFO);
        if ($bankOrder instanceof BankXac) {
            if ($bankOrder->canReCharge()) {
                $bankOrder->status = self::STAT_PROCESS;
                $bankOrder->save();
            ...
            $bankOrderRows = self::updateForCharge(array(BankXacAccountTable::ACCOUNT_USSD_TOPUP), $limit);
        }
        ...
        foreach ($bankOrderRows as $bankOrder) {
            try {
                ...
                if (preg_match($topupRegex, $txnDesc, $matches)) {
                    ...
                    # TEST hiih dugaaruudiig l zuvshuuruv
                    //if (in_array($phoneNumber, array('94300074', '94300115'))) {
                    $result = RtcgwGateway::chargeTopup($phoneNumber, $card, "bankgw_xac");
                    //}
                    if (isset($result['Code']) && $result['Code'] == 0) {
                        $status = BankpaymentTable::STAT_SUCCESS;
                    } else {
                    ...
                    # TEST hiih dugaaruudiig l zuvshuuruv
                    //if (in_array($phoneNumber, array('94300074', '94300115'))) {
                    $result = SapcGateway::chargeFreePackage($phoneNumber, $card, $logger, "XACBANK");
                    //}
                    if (isset($result['Code']) && $result['Code'] == 0) {
                        $status = BankpaymentTable::STAT_SUCCESS;
                    } else {
        ...
        $topupLogger->log('==========Ended bankPaymentTopupSapc Xac================', sfFileLogger::INFO);
        return TRUE;
    }
    ...
    public static function sendPaymentVat($bankOrder)
    {
        ...
        $logger = new sfFileLogger(new sfEventDispatcher(), array('file' => sfConfig::get('sf_log_dir') . '/PaymentVAT/xac/PaymentVAT_'.date('Y-m-d').'.log'));
        try {
            $bill = $phoneNumber = $contractNumber = $contractName = $statusComment = null;
            $contractAmount = $billCycle = $paymentCode = 0;
            ...
            if ($bankTransaction && $bankpayment) {
                $logger->log($bankOrder->order_id . ' PaymentVAT:XAC bankpayment: ' . $bankpayment->id. ', $bankTransaction: ' . $bankTransaction->id, sfFileLogger::INFO);
    ...
    {
        $logger = new sfFileLogger(new sfEventDispatcher(), array('file' => sfConfig::get('sf_log_dir') . '/bankPaymentFromTransaction/xac/bankPaymentFromTransaction_'.date('Y-m-d').'.log'));
        $logger->log('============Started bankPaymentFromTransaction XAC================', sfFileLogger::INFO);
        if ($bankOrder instanceof BankXac) {
            if ($bankOrder->canReCharge()) {
                $bankOrder->status = self::STAT_PROCESS;
                $bankOrder->save();
        ...
        $logger->log('============Ended bankPaymentFromTransaction XAC================', sfFileLogger::INFO);
        return TRUE;
    }
    ...
    public static function loyaltyBankpayment($bankOrder = null, $limit = 50)
    {
        if ($bankOrder instanceof BankXac) {
            if (in_array($bankOrder->status, array(
                BankXacTable::STAT_FAILED,
                BankXacTable::STAT_PROCESS,
                BankXacTable::STAT_FAILED_CHARGE,
                BankXacTable::STAT_FAILED_OUTCOME))) {
                $bankOrder->status = self::STAT_PROCESS;
                $bankOrder->save();
            }
            $bankOrderRows = array($bankOrder);
            ...
            $bankOrderRows = self::updateForCharge(array(BankXacAccountTable::ACCOUNT_MOBIFINANCE_CANDY, BankXacAccountTable::ACCOUNT_CANDY_CASHIN), $limit);
        }
        foreach ($bankOrderRows as $bankOrder) {
            try {
                if (in_array($bankOrder->status, array(self::STAT_FAILED_CHARGE, self::STAT_FAILED))) {
                ...
                if ($chargeNumber) {
                    ...
                    if ($isQpay) {
                        ...
                        $bankOrder->setStatus(BankXacTable::STAT_SUCCESS);
                        $bankOrder->save();
                        continue;
                    }
                    $isCandyAgent = LoyaltyCharge::checkCandyAgent($txnDesc);
                    $isCandyAccount = LoyaltyCharge::checkCandyAccount($bankOrder['bank_account'], 'XAC');
                    if ($bankOrder['bank_account'] == BankXacAccountTable::ACCOUNT_MOBIFINANCE_CANDY) {
