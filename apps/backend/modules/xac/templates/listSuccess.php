<form id="form" action="<?php echo url_for('@bank_xac_list') ?>" autocomplete="off">
    <fieldset>
        <legend>ХАЙЛТ</legend>
        ...
        <a href="<?php echo url_for('@bank_xac_list_excel?' . $urlParams) ?>"><input type="button" value="Excel"/></a>
        <?php endif; ?>
    </fieldset>
</form>
...
<table>
    ...
    <tbody>
        <?php foreach ($pager->getResults() as $xac): ?>
            <?php $status = BankXacTable::getStatusName($xac->status, BankXacTable::TYPE_DEALER, true); ?>
            <tr>
                <td><?php echo $xac->order_id ?></td>
                <td><?php echo $xac->bank_account ?></td>
                <td><?php echo $xac->charge_mobile ?></td>
                <td><?php echo $xac->order_mobile ?></td>
                <td align="center"><?php echo $xac->order_type ?></td>
                <td><?php echo $xac->charge_amount ?></td>
                <td><?php echo $xac->order_amount ?></td>
                <td><?php echo ($xac->charge_amount - $xac->order_amount) ?></td>
                <td align="center">
                    <?php echo link_to($status, '@bank_xac_status?id=' . $xac->id, array('rel' => 'facebox')) ?>
                    <span title="Оролдлогын тоо">
                        (<?php echo $xac->try_count ?>)
                    </span>
                </td>
                <td align="center"><?php echo $xac->order_date ?></td>
                <td align="center"><?php echo $xac->created_at ?></td>
                <td align="center"><?php echo $xac->updated_at ?></td>
            </tr>
        <?php endforeach; ?>
    </tbody>
    <tfoot>
...
<?php echo pager_navigation($pager, AppTools::getQueryString('@bank_xac_list')) ?>
