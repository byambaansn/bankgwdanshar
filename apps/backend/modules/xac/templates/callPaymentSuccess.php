...
<form id="form" action="<?php echo url_for('@bank_xac_call_payment_list') ?>" autocomplete="off">
    <fieldset>
        <legend>ХАЙЛТ</legend>
        ...
        <a href="<?php echo url_for('@bank_xac_call_payment_excel?' . $urlParams) ?>"><input type="button" value="Excel"/></a>
        <?php endif; ?>
    </fieldset>
</form>
...
<table>
    ...
    <tbody>
        <?php foreach ($pager->getResults() as $xac): ?>
            <?php $status = BankXacTable::getStatusName($xac->status, BankXacTable::TYPE_CALLPAYMENT, true); ?>
            <tr>
                ...
                <td align="center">
                    <?php echo link_to($status, '@bank_xac_call_payment_status?id=' . $xac->id, array('rel' => 'facebox')) ?>
...
<?php echo pager_navigation($pager, AppTools::getQueryString('@bank_xac_call_payment_list')) ?>
