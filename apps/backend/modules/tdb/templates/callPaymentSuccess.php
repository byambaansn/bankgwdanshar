...
<form id="form" action="<?php echo url_for('@bank_tdb_call_payment_list') ?>" autocomplete="off">
    <fieldset>
        <legend>ХАЙЛТ</legend>
        ...
        <a href="<?php echo url_for('@bank_tdb_call_payment_excel?' . $urlParams) ?>"><input type="button" value="Excel"/></a>
        <?php endif; ?>
    </fieldset>
</form>
...
<table>
    ...
    <tbody>
        <?php foreach ($pager->getResults() as $tdb): ?>
            <?php $status = BankTdbTable::getStatusName($tdb->status, BankTdbTable::TYPE_CALLPAYMENT, true); ?>
            <tr>
                ...
                <td align="center">
                    <?php echo link_to($status, '@bank_tdb_call_payment_status?id=' . $tdb->id, array('rel' => 'facebox')) ?>
...
<?php echo pager_navigation($pager, AppTools::getQueryString('@bank_tdb_call_payment_list')) ?>
