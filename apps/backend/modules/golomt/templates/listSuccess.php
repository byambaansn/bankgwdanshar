...
<form id="form" action="<?php echo url_for('@bank_golomt_list') ?>" autocomplete="off">
    <fieldset>
        <legend>ХАЙЛТ</legend>
        ...
        <a href="<?php echo url_for('@bank_golomt_list_excel?' . $urlParams) ?>"><input type="button" value="Excel"/></a>
        <?php endif; ?>
    </fieldset>
</form>
...
<table>
    ...
    <tbody>
        <?php foreach ($pager->getResults() as $golomt): ?>
            <?php $status = BankGolomtTable::getStatusName($golomt->status, BankGolomtTable::TYPE_DEALER, true); ?>
            <tr>
                <td><?php echo $golomt->order_id ?></td>
                <td><?php echo $golomt->bank_account ?></td>
                <td><?php echo $golomt->charge_mobile ?></td>
                <td><?php echo $golomt->order_mobile ?></td>
                <td align="center"><?php echo $golomt->order_type ?></td>
                <td><?php echo $golomt->charge_amount ?></td>
                <td><?php echo $golomt->order_amount ?></td>
                <td><?php echo ($golomt->charge_amount - $golomt->order_amount) ?></td>
                <td align="center">
                    <?php echo link_to($status, '@bank_golomt_status?id=' . $golomt->id, array('rel' => 'facebox')) ?>
                    <span title="Оролдлогын тоо">
                        (<?php echo $golomt->try_count ?>)
                    </span>
                </td>
                <td align="center"><?php echo $golomt->order_date ?></td>
                <td align="center"><?php echo $golomt->created_at ?></td>
                <td align="center"><?php echo $golomt->updated_at ?></td>
            </tr>
        <?php endforeach; ?>
    </tbody>
    <tfoot>
...
<?php echo pager_navigation($pager, AppTools::getQueryString('@bank_golomt_list')) ?>
