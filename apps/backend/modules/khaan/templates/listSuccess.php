...
<form id="form" action="<?php echo url_for('@bank_khaan_list') ?>" autocomplete="off">
    <fieldset>
        <legend>ХАЙЛТ</legend>
        ...
        <a href="<?php echo url_for('@bank_khaan_list_excel?' . $urlParams) ?>"><input type="button" value="Excel"/></a>
        <?php endif; ?>
    </fieldset>
</form>
...
<table>
    ...
    <tbody>
        <?php foreach ($pager->getResults() as $khaan): ?>
            <?php $status = BankKhaanTable::getStatusName($khaan->status, BankKhaanTable::TYPE_DEALER, true); ?>
            <tr>
                <td><?php echo $khaan->order_id ?></td>
                <td><?php echo $khaan->bank_account ?></td>
                <td><?php echo $khaan->related_account ?></td>
                <td><?php echo $khaan->charge_mobile ?></td>
                <td><?php echo $khaan->order_mobile ?></td>
                <td align="center"><?php echo $khaan->order_type ?></td>
                <td><?php echo $khaan->charge_amount ?></td>
                <td><?php echo $khaan->order_amount ?></td>
                <td><?php echo ($khaan->charge_amount - $khaan->order_amount) ?></td>
                <td align="center">
                    <?php echo link_to($status, '@bank_khaan_status?id=' . $khaan->id, array('rel' => 'facebox')) ?>
...
<?php echo pager_navigation($pager, AppTools::getQueryString('@bank_khaan_list')) ?>
