...
<form id="form" action="<?php echo url_for('@bank_capital_list') ?>" autocomplete="off">
    <fieldset>
        <legend>ХАЙЛТ</legend>
        ...
        <a href="<?php echo url_for('@bank_capital_list_excel?' . $urlParams) ?>"><input type="button" value="Excel"/></a>
        <?php endif; ?>
    </fieldset>
</form>
...
<table>
    ...
    <tbody>
        <?php foreach ($pager->getResults() as $capital): ?>
            <?php $status = BankCapitalTable::getStatusName($capital->status, BankCapitalTable::TYPE_DEALER, true); ?>
            <tr>
                <td><?php echo $capital->order_id ?></td>
                <td><?php echo $capital->bank_account ?></td>
                <td><?php echo $capital->charge_mobile ?></td>
                <td><?php echo $capital->order_mobile ?></td>
                <td align="center"><?php echo $capital->order_type ?></td>
                <td><?php echo $capital->charge_amount ?></td>
                <td><?php echo $capital->order_amount ?></td>
                <td><?php echo ($capital->charge_amount - $capital->order_amount) ?></td>
                <td align="center">
                    <?php echo link_to($status, '@bank_capital_status?id=' . $capital->id, array('rel' => 'facebox')) ?>
                    <span title="Оролдлогын тоо">
                        (<?php echo $capital->try_count ?>)
                    </span>
                </td>
                <td align="center"><?php echo $capital->order_date ?></td>
                <td align="center"><?php echo $capital->created_at ?></td>
                <td align="center"><?php echo $capital->updated_at ?></td>
            </tr>
        <?php endforeach; ?>
    </tbody>
    <tfoot>
...
<?php echo pager_navigation($pager, AppTools::getQueryString('@bank_capital_list')) ?>
