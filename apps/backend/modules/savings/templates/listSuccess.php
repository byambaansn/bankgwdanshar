...
<form id="form" action="<?php echo url_for('@bank_savings_list') ?>" autocomplete="off">
    <fieldset>
        <legend>ХАЙЛТ</legend>
        ...
        <a href="<?php echo url_for('@bank_savings_list_excel?' . $urlParams) ?>"><input type="button" value="Excel"/></a>
        <?php endif; ?>
    </fieldset>
</form>
...
<table>
    ...
    <tbody>
        <?php foreach ($pager->getResults() as $savings): ?>
            <?php $status = BankSavingsTable::getStatusName($savings->status, BankSavingsAccountTable::ACCOUNT_DEALER, true); ?>
            <tr>
                ...
                <td><?php echo ($savings->charge_amount - $savings->order_amount) ?></td>
                <td align="center">
                    <?php echo link_to($status, '@bank_savings_status?id=' . $savings->id, array('rel' => 'facebox')) ?>
...
<?php echo pager_navigation($pager, AppTools::getQueryString('@bank_savings_list')) ?>
