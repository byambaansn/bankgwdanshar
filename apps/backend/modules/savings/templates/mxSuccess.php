<form id="form" action="<?php echo url_for('@bank_savings_mx_list') ?>" autocomplete="off">
    <fieldset>
        <legend>ХАЙЛТ</legend>
        ...
        <label for="chargedMobile">Хүлээн авагч дугаар:</label>
        <input type="text" name="chargedMobile" value="<?php echo $chargedMobile ?>" />
        <span class="desc"></span>
        <br />

        <label for="orderedMobile">Шилжүүлэгч дугаар:</label>
        ...
        <a href="<?php echo url_for('@bank_savings_mx_list_excel?' . $urlParams) ?>"><input type="button" value="Excel"/></a>
        <?php endif; ?>
    </fieldset>
</form>

<div class="info"><b class="red">C</b> - Хүлээн авагч дугаар <b class="red">D</b> - Шилжүүлэгч дугаар</div>
...
<table>
    ...
    <tbody>
        <?php foreach ($pager->getResults() as $savings): ?>
            <?php $status = BankSavingsTable::getStatusName($savings->status, BankSavingsAccountTable::ACCOUNT_MOBIXPRESS, true); ?>
            <tr>
                <td><?php echo $savings->order_id ?></td>
                <td><?php echo $savings->bank_account ?></td>
                <td><?php echo $savings->charge_mobile ?></td>
                <td><?php echo $savings->order_mobile ?></td>
                <td align="center"><?php echo $savings->order_type ?></td>
                <td><?php echo $savings->charge_amount ?></td>
                <td align="center">
                    <?php echo link_to($status, '@bank_savings_mx_status?id=' . $savings->id, array('rel' => 'facebox')) ?>
...
<?php echo pager_navigation($pager, AppTools::getQueryString('@bank_savings_mx_list')) ?>
