...
<form id="form" action="<?php echo url_for('@bank_mobixpress_list') ?>" autocomplete="off">
    <fieldset>
        <legend>ХАЙЛТ</legend>
        ...
        <a href="<?php echo url_for('@bank_mobixpress_list_excel?' . $urlParams) ?>"><input type="button" value="Excel"/></a>
        <?php endif; ?>
    </fieldset>
</form>
...
<table>
    ...
    <tbody>
        <?php foreach ($pager->getResults() as $mobixpress): ?>
            <?php $status = BankMobixpressTable::getStatusName($mobixpress->status, 1, true); ?>
            <tr>
                <td><?php echo $mobixpress->order_id ?></td>
                <td><?php echo $mobixpress->bank_account ?></td>
                <td><?php echo $mobixpress->charge_mobile ?></td>
                <td><?php echo $mobixpress->order_mobile ?></td>
                <td align="center"><?php echo $mobixpress->order_type ?></td>
                <td><?php echo $mobixpress->charge_amount ?></td>
                <td><?php echo $mobixpress->order_amount ?></td>
                <td><?php echo ($mobixpress->charge_amount - $mobixpress->order_amount) ?></td>
                <td align="center">
                    <?php echo link_to($status, '@bank_mobixpress_status?id=' . $mobixpress->id, array('rel' => 'facebox')) ?>
                    <span title="Оролдлогын тоо">
                        (<?php echo $mobixpress->try_count ?>)
                    </span>
                </td>
                <td align="center"><?php echo $mobixpress->created_at ?></td>
                <td align="center"><?php echo $mobixpress->updated_at ?></td>
            </tr>
        <?php endforeach; ?>
    </tbody>
    <tfoot>
...
<?php echo pager_navigation($pager, AppTools::getQueryString('@bank_mobixpress_list')) ?>
