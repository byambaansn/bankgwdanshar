...
<form id="form" action="<?php echo url_for('@bank_capitron_list') ?>" autocomplete="off">
    <fieldset>
        <legend>ХАЙЛТ</legend>
        ...
        <a href="<?php echo url_for('@bank_capitron_list_excel?' . $urlParams) ?>">
...
<table>
    ...
    <tbody>
        <?php foreach ($pager->getResults() as $capitron): ?>
            <?php $status = BankCapitronTable::getStatusName($capitron->status, BankCapitronTable::TYPE_CALLPAYMENT, true); ?>
            <tr>
                <td><?php echo $capitron->order_id ?></td>
                <td><?php echo $capitron->bank_account ?></td>
                <td><?php echo $capitron->charge_mobile ?></td>
                <td><?php echo $capitron->order_mobile ?></td>
                <td align="center"><?php echo $capitron->order_type ?></td>
                <td><?php echo $capitron->charge_amount ?></td>
                <td><?php echo $capitron->order_amount ?></td>
                <td><?php echo ($capitron->charge_amount - $capitron->order_amount) ?></td>
                <td align="center">
                    <?php echo link_to($status, '@bank_capitron_status?id=' . $capitron->id, array('rel' => 'facebox')) ?>
                    <span title="Оролдлогын тоо">
                        (<?php echo $capitron->try_count ?>)
                    </span>
                </td>
                <td align="center"><?php echo $capitron->order_date ?></td>
                <td align="center"><?php echo $capitron->created_at ?></td>
                <td align="center"><?php echo $capitron->updated_at ?></td>
            </tr>
        <?php endforeach; ?>
    </tbody>
    <tfoot>
...
<?php echo pager_navigation($pager, AppTools::getQueryString('@bank_capitron_list')) ?>
